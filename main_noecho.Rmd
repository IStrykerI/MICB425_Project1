...
knitr::opts_chunk$set(fig.width=8, fig.height=6, echo=FALSE, warning=FALSE, include=TRUE, tidy = TRUE, message=FALSE)
```
...
We first estimate the overall taxa diversity using Shannon's diversity index as shown in [Figure 1](#Figure1).
...
To understand the number of taxa associated with the Phylum rank, we found that there are around `r get_taxa_unique(physeq = thirdMTaxa, taxonomic.rank = "Phylum")` taxa in mothur in compare to  `r get_taxa_unique(physeq = thirdMTaxa, taxonomic.rank = "Phylum")` taxa in QIIME2.
