...
exclude ASVs that are not observed for more than \(4\) samples; ii)-
prune samples and ASVs with unknown values, such as
\texttt{unclassified} value; and iii)- any phylum fail to have more than
\(5\) ASVs should be trimmed. The codes used for applying the three
rules are:
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# General linear model for each ASV}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Abundance of ASVs within unclassified domain across depth}
...
\CommentTok{# Abundance of ASVs within unclassified depth by colour}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Abundance of ASVs within Planctomyces genus across depth}
...
\CommentTok{# Abundance of ASVs within Planctomyces genus by colour}
