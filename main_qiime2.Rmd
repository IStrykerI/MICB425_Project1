...
Next, we perform a series of filterings according to three rules: i)- exclude ASVs that are not observed for more than $4$ samples; ii)- prune samples and ASVs with unknown values, such as ``unclassified`` value; and iii)- any phylum fail to have more than $5$ ASVs should be trimmed. The codes used for applying the three rules are:
```{r}
# First rule
...
rarefiedRichAlpha <- full_join(rownames_to_column(rarefiedRich), 
                        rownames_to_column(data.frame(sample_data(rarefiedPer))), by = "rowname")
...
Finally, we settled on performing experimental analysis at \emph{Planctomyces} genus level and its associated ASVs.
```{r}
...
```{r}
#General linear model for each ASV
...
```{r}
# Abundance of ASVs within unclassified domain across depth
...
```{r}
# Abundance of ASVs within Planctomyces genus across depth
...
# Abundance of ASVs within Planctomyces genus by colour
